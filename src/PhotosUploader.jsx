...
export default function PhotosUploader({addedPhotos,onChange}){
    ...
    async function addPhotoByLink(ev){
        ...
        onChange(prev => { //The 'prev' arg represents the current state val(prev is just a conventional name), and return a new arr using the spread operator '[...prev]' to copy the eles from the prev state array 
                    ...
                    <div className=" mt-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 ">
                        {addedPhotos.length > 0 && addedPhotos.map((ele) => (
                            <div className="flex h-32 relative" key={ele}>
                                ...
                                <button onClick={(ev) => selectAsMainPhoto(ev,ele)} className="cursor-pointer absolute bottom-1 left-1 text-white bg-black bg-opacity-50 rounded-2xl p-1">
                                    {ele === addedPhotos[0] && (
